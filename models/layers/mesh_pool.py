...
class MeshPool(nn.Module):
    ...
    def __pool_main(self, mesh_index):
        ...
        # print("池化前边的数量：", mesh.edges_count)
        queue = self.__build_queue(mesh.edges_count)
        ...
        while mesh.edges_count > self.__out_target:
            if not queue:
                print("--------------------")
                break
            edge_id = queue.pop()  # 直接从打乱的列表中取出一个边的索引
            if mask[edge_id]:
                self.__pool_edge(mesh, edge_id, mask, edge_groups)
        # print("mesh.clean之前边的数量：", mesh.edges_count)
    ...
    def __build_queue(self, edges_count):
        # 创建包含所有边索引的列表
        edge_ids = list(range(edges_count))
        # 随机打乱边的顺序
